...
struct Shader : IShader {
  ...
  mat<2, 3> varying_uv;   // triangle uv coordinates, written by the vertex
                          // shader, read by the fragment shader
  ...
  virtual bool fragment(const vec3 &bar, const TGAColor &gl_FragColor) {
    ...
    mat<3, 3> AI = mat<3, 3>{
        {view_tri.col(1) - view_tri.col(0), view_tri.col(2) - view_tri.col(0),
         bn}}.invert();
    ...
    vec3 n =
        (B * model.normal(uv)).normalized();    // transform the normal from the
                                                // texture to the tangent space
    double diff = std::max(0., n * uniform_l);  // diffuse light intensity
    vec3 r =
        (n * (n * uniform_l) * 2 - uniform_l)
            .normalized();  // reflected light direction, specular mapping is
                            // described here:
                            // https://github.com/ssloy/tinyrenderer/wiki/Lesson-6-Shaders-for-the-software-renderer
    double spec = std::pow(
        std::max(-r.z, 0.),
        5 + sample2D(model.specular(),
                     uv)[0]);  // specular intensity, note that the camera lies
                               // on the z-axis (in view), therefore simple -r.z

    TGAColor c = sample2D(model.diffuse(), uv);
    for (int i : {0, 1, 2})
      gl_FragColor[i] = std::min<int>(
          10 + c[i] * (diff + spec),
          255);  // (a bit of ambient light, diff + spec), clamp the result
...
int main(int argc, char **argv) {
  ...
  viewport(width / 8, height / 8, width * 3 / 4,
           height * 3 / 4);           // build the Viewport matrix
  ...
  for (int m = 1; m < argc; m++) {  // iterate through all input objects
    ...
    for (int i = 0; i < model.nfaces(); i++) {  // for every triangle
      vec4 clip_vert[3];  // triangle coordinates (clip coordinates), written by
                          // VS, read by FS
      for (int j : {0, 1, 2})
        shader.vertex(
            i, j,
            clip_vert[j]);  // call the vertex shader for each triangle vertex
